/**
 * COMMAND: dds XP for users who hosted events and updated to our Cloud MongoDB Database, this command is protected so only those with the 'Admin' role can run it.
 */
// Requirements
...
exports.run = async (client, message) => {
  ...
  if (checkRoles) {
    ...
    if (args.length === 0) {
      message.reply(
        " you forgot to say who you're awarding xp to, try like this: `o!hosted RSN`"
      );
    }
        ...
        function (error, data) {
          ...
          // If we can succsesfully connect to DB...
            ...
            // Add XP if user hasn't talked on discord today.
              ...
              if (data.hostedEvent === todaysDate) {
                message.channel.send(
                  '**' +
                    readable +
                    '** has already hosted an event today and cannot gain anymore XP.'
                );
              } else {
                // Find users current XP
                let newXp = data.xp + 30;
                ...
                XP.findOneAndUpdate(
                  ...
                  function (err, result) {
                    ...
                    // Succsessfully updated user in DB
                      ...
                      message.channel.send(
                        'Awarded XP for hosting an event to ' +
                          readable +
                          ' on ' +
                          todaysDate +
                          '\n\n' +
