/**
 * COMMAND: Adds XP to a user for winning a weekly competition, command is locked to only those with the 'Admin' role and therefore checks against this and takes a query of {USER} before sending data to cloud-based mongoDB database
 */
// Requirements
          ...
          // If we can succsesfully connect to DB...
            ...
            if (!data) {
              message.reply(
                ' cannot find **' +
                  readable +
                  '** in my database. Run `o!addplayer {USER}` to add a new player to the Database! '
              );
            }
              ...
              XP.findOneAndUpdate(
                ...
                function (err, data) {
                  ...
                  // Succsessfully updated user in DB
                    ...
                    message.channel.send(
                      'Awarded XP for placing 1st, 2nd or 3rd in SOTW to ' +
                        readable +
                        ' on ' +
                        todaysDate +
                        '\n\n' +
