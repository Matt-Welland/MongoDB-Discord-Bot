/**
 * COMMAND: Admin only command to grant XP to a user if they talk on RuneScape.com, takes the query {USER} and checks if the user running the command has the role 'Admin', if criteria is met update "XP" total on Cloud MongoDB
 */
// Requirements
...
exports.run = async (client, message) => {
  ...
  if (checkRoles) {
    ...
    if (args.length === 0) {
      message.reply(
        " you forgot to say who you're awarding xp to, try like this: `o!talkedxp RSN`"
      );
    }
      ...
      XP.findOne(
        ...
        function (error, data) {
          ...
          // If we can succsesfully connect to DB...
            ...
            // Add XP if user hasn't talked today.
            else {
              // If the person has already talked today
              if (data.talkedGame === todaysDate) {
                message.channel.send(
                  '**' +
                    readable +
                    '** has already talked in the CC today and cannot gain anymore XP.'
                );
              } else {
                ...
                XP.findOneAndUpdate(
                  ...
                  function (err, result) {
                    ...
                    // Succsessfully updated user in DB
                      ...
                      message.channel.send(
                        'Awarded XP for talking in CC to ' +
                          readable +
                          ' on ' +
                          todaysDate +
                          '\n\n' +
